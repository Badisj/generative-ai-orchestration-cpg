...
class YogurtFeatureEngineer:
    ...
    def get_feature_groups(self) -> Dict[str, List[str]]:
        """
        Return dictionary of feature groups for easy model building
        
        Returns:
            Dictionary with feature group names and corresponding column lists
        """
        
        feature_groups = {
            'protein_features': [
                'protein_total', 'casein_equivalent', 'whey_equivalent', 
                'casein_whey_ratio', 'fresh_vs_powder_ratio', 'protein_source_diversity'
            ],
            
            'fat_features': [
                'fat_total', 'fat_from_AMF', 'fat_from_cream', 'fat_from_powder',
                'fat_from_fresh_milk', 'added_fat_ratio', 'fat_source_diversity'
            ],
            
            'carbohydrate_features': [
                'sugar_total', 'lactose_natural', 'sugar_added', 'added_vs_natural_sugar',
                'fermentable_substrate'
            ],
            
            'flavor_color_features': [
                'is_strawberry', 'is_chocolate', 'is_plain', 'flavor_intensity_total',
                'color_L_predicted', 'color_a_predicted', 'color_b_predicted',
                'strawberry_harmony', 'chocolate_harmony'
            ],
            
            'process_features': [
                'pasteurization_intensity', 'homog_energy_density', 'pressure_ratio',
                'process_energy_total'
            ],
            
            'texture_features': [
                'texture_synergy_score', 'protein_network_strength', 'fat_lubrication_factor',
                'stabilizer_to_protein_ratio', 'particle_uniformity_index'
            ],
            
            'fermentation_features': [
                'acidification_potential', 'culture_substrate_quality', 'pH_buffering_capacity'
            ],
            
            'particle_features': [
                'fat_globule_size_predicted', 'protein_aggregation_potential',
                'initial_globule_size_weighted', 'homog_challenge_index'
            ],
            
            'nutritional_features': [
                'vitamin_fortification_intensity', 'fat_soluble_vitamin_bioavailability',
                'mineral_content_natural', 'taste_balance_score'
            ],
            
            'quality_features': [
                'formulation_complexity', 'premium_ingredient_score', 
                'process_optimization_score', 'emulsion_stability_predictor'
            ]
        }
    ...
    def get_feature_descriptions(self) -> Dict[str, str]:
        """
        Return descriptions of key engineered features
        ...
        descriptions = {
            'texture_synergy_score': 'Combined protein-fat-stabilizer interaction effect on texture',
            'acidification_potential': 'Culture fermentation potential considering substrates and buffering',
            'casein_whey_ratio': 'Ratio of casein to whey proteins affecting gel strength',
            'fat_globule_size_predicted': 'Predicted fat globule size after homogenization (μm)',
            'homog_energy_density': 'Total homogenization energy (bar × KL)',
            'protein_network_strength': 'Weighted protein contribution to gel network',
            'emulsion_stability_predictor': 'Combined fat-emulsifier-process stability score',
            'strawberry_harmony': 'Flavor-color matching score for strawberry variants',
            'chocolate_harmony': 'Flavor-color matching score for chocolate variants',
            'process_optimization_score': 'Overall process parameter optimization indicator',
            'premium_ingredient_score': 'Score based on premium ingredient usage',
            'taste_balance_score': 'Balance between bitterness and masking flavors/sugars'
        }
...
def main():
    ...
    for group_name, features in feature_groups.items():
        ...
        print(f"  {features[:3]}..." if len(features) > 3 else f"  {features}")
