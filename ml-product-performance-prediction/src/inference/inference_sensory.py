...
###################################### MAIN ######################################
# Convert percentage and measurement columns to float
float_cols = [
  'cocoa_beans', 'cocoa_butter', 
  'palm_oil', 'conching_temperature_c', 'conching_time_min', 
  'cooling_rate_c_per_min', 'mixing_speed_rpm', 'refining_passes', 
  'grinding_fineness_micron', 'processing_humidity_pct', 'ambient_temperature_c',
  'whole_milk_powder', 'beetroot_sugar', 'soy_lecithin', 
  'artificial_vanilla_aroma', 'natural_vanilla_aroma', 'natural_flavor',
  'Energy_kcal', 'protein', 'fat', 'SFA', 'carbohydrates', 'Total_sugars', 
  'tempering_temperature_c'
]

# Apply conversions
plp_df[float_cols] = plp_df[float_cols].astype(float)

# Compute values and output dataset
df_enhanced = process_complete_feature_engineering(plp_df)
plp_df = df_enhanced.copy()